...
assert(adConfig.enabled === false && adConfig.publisher_id === 'ca-pub-2476023536699107' && adConfig.certified_cmp_ready === false && adConfig.operator_identity_confirmed === false, '확인되지 않은 광고 계정 또는 운영 조건을 활성화함');
assert(adReadiness.code.verified === 16 && adReadiness.code.live === 16 && adReadiness.code.percent === 100 && adReadiness.external_activation.done === 1, '광고 코드 준비와 확인된 ads.txt 증거 대조 실패');
assert(adStageTwo.code.verified === 24 && [0,24].includes(adStageTwo.code.live) && adStageTwo.code.percent === 100 && adStageTwo.external_activation.done === 1 && adStageTwo.active_providers.external === 0, '광고 2단계 코드·ads.txt·실제 제공자 운영을 섞음');
assert(readiness.advertising_stage_two.code_verified === adStageTwo.code.verified && readiness.advertising_stage_two.external_done === 1, '전체 진척과 광고 2단계 현황 대조 실패');
assert(adStageThree.code.verified === 32 && [0,32].includes(adStageThree.code.live) && adStageThree.code.percent === 100 && adStageThree.external_activation.done === 1 && adStageThree.supply_chain.done === 1 && adStageThree.active_operations.live_allocation_percent === 0, '광고 3단계 코드·ads.txt·실제 고급 운영을 섞음');
assert(readiness.advertising_stage_three.code_verified === adStageThree.code.verified && readiness.advertising_stage_three.external_done === 1 && readiness.advertising_stage_three.supply_chain_done === 1, '전체 진척과 광고 3단계 현황 대조 실패');
assert(commercialLaunch.technical.total === 12 && commercialLaunch.technical.verified === 12 && commercialLaunch.technical.percent === 100 && commercialLaunch.external.total === 12 && commercialLaunch.external.done === 2 && commercialLaunch.external.cards.filter((card) => card.status === 'VERIFIED_EXTERNAL').length === 2, '무료 상용화 기술 준비와 외부 증거를 섞음');
assert(commercialEvidence.site.adsense_registered === true && commercialEvidence.ads_txt.publisher_match === true && commercialEvidence.adsense.site_approved === false, '실제 AdSense 준비 중 상태 또는 확인된 웹사이트·ads.txt 증거 오류');
assert(readiness.commercial_launch.technical_verified === 12 && readiness.commercial_launch.external_done === 2, '전체 진척과 무료 상용화 현황 대조 실패');
