...
const target = await fetch(`http://127.0.0.1:${port}/json/new?${encodeURIComponent(`${base}?device_check=v029#home`)}`, { method: 'PUT' }).then((response) => response.json());
...
try {
  ...
  await send('Page.navigate', { url: `${base}?device_check=v029#home` });
  ...
  console.error('[a56] 공개 v0.29 준비 완료');
  ...
  console.log(JSON.stringify({ verdict: 'MEASURED', device: 'A56', release: 'v0.29', ...result }, null, 2));
